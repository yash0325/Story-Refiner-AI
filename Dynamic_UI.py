...
with st.form("connection_form"):
    jira_host = st.text_input("Jira Host URL (e.g. https://yourdomain.atlassian.net)")
...
if submitted:
    ...
    st.session_state["connected"] = False  # To be set True after connecting

# Restore from session if present
jira_host = st.session_state.get("jira_host", "")
jira_email = st.session_state.get("jira_email", "")
jira_api_token = st.session_state.get("jira_api_token", "")
jira_project_key = st.session_state.get("jira_project_key", "")
openai_api_key = st.session_state.get("openai_api_key", "")

def connect_to_jira(jira_host, jira_email, jira_api_token):
    try:
...
# Step 2: Establish connection and set flag
jira = None
if jira_host and jira_email and jira_api_token and jira_project_key and openai_api_key:
    jira = connect_to_jira(jira_host, jira_email, jira_api_token)
    if jira is not None:
        st.session_state["connected"] = True
        st.success(f"Connected to JIRA: {jira_project_key}")
        os.environ["OPENAI_API_KEY"] = openai_api_key  # For LangChain (optional, since we're passing key)
    else:
        st.session_state["connected"] = False
else:
    st.info("Please fill in all connection fields above and click Connect.")

# Step 3: Show the app only if connected
if st.session_state.get("connected", False) and jira is not None:
